...
- для русских субтитров использует `player_client=web`, `--ignore-no-formats-error` и локальный HTTP PO Token Provider `bgutil`; русский запрос выполняется один раз, а подробная диагностика включается отдельно;
...
Switch-параметр `$RusubVerboseDiag` также выключен по умолчанию. Он добавляет `yt-dlp --verbose` только к запросу русских субтитров и сам по себе не включает их скачивание.

| Параметры | Поведение |
|---|---|
| без параметров | Русские субтитры отключены |
| `-AddRusub` | Один русский запрос через `player_client=web`, обычный вывод |
| `-AddRusub -RusubVerboseDiag` | Один русский запрос через `player_client=web`, подробная диагностика |
| только `-RusubVerboseDiag` | Диагностика не действует, потому что русская ветка отключена |
...
Диагностический запуск с русскими субтитрами:
...
- PowerShell-сценарий сохраняет контролируемые повторы для оригинальных субтитров, но выполняет только одну диагностическую попытку русских субтитров; ошибка субтитров не удаляет успешно скачанное видео.
