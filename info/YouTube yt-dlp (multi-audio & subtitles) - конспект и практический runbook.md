...
# Дополнение: `download_yt-dlp_ru_v1_mp3.bat`

## Назначение и проверенное поведение

Новый сценарий сохраняет звуковые дорожки YouTube как отдельные MP3-файлы:

```text
Original audio -> <title> [<video-id>] [Original].mp3
Russian audio  -> <title> [<video-id>] [Russian].mp3
                  только если original != ru и отдельная ru-дорожка существует
```

Кодирование выполняется через `ffmpeg`:

```text
-c:a libmp3lame -b:a 320k
```

То есть создаётся MP3 с постоянным целевым битрейтом `320 kb/s` (`CBR`). Перекодирование не повышает реальное качество исходной дорожки, если YouTube отдал аудио с меньшим эффективным битрейтом.

> [!success] Observed
> Реальное скачивание с YouTube через `download_yt-dlp_ru_v1_mp3.bat` подтверждено. Original MP3 создаётся; логика дополнительной Russian-дорожки применяется только при наличии отдельного русского формата.

## Почему появлялось предупреждение `/ping`

Наблюдавшееся сообщение:

```text
WARNING: [youtube] [pot:bgutil:http] Error reaching GET http://127.0.0.1:4416/ping (caused by TransportError).
```

Сам BAT-файл не выполнял HTTP-запрос к `/ping`. Причина находилась на уровне plugin discovery:

```text
yt-dlp запускается
  -> автоматически обнаруживает установленный youtubepot-bgutilhttp plugin
  -> plugin проверяет HTTP provider через 127.0.0.1:4416/ping
  -> provider не запущен
  -> появляется предупреждение
```

Для MP3-сценария PO Token Provider специально не требуется: он был добавлен для отдельной нестабильной ветки получения русских translated subtitles. Поэтому каждый вызов `yt-dlp` в MP3-скрипте теперь начинается так:

```bat
yt-dlp.exe --no-plugin-dirs ...
```

`--no-plugin-dirs` очищает каталоги поиска плагинов, включая каталоги по умолчанию. В результате `bgutil` plugin не загружается, `/ping` не вызывается и связанное с ним предупреждение не возникает.

> [!important] Граница ответственности
> `--no-plugin-dirs` применяется только внутри `download_yt-dlp_ru_v1_mp3.bat`. Субтитровый `download_yt-dlp_subs_v7.ps1` продолжает использовать `bgutil` в ветке `-AddRusub`, где provider действительно нужен.

> [!note] Почему не `--ignore-config`
> `--ignore-config` отключил бы загрузку конфигурационных файлов целиком. Это более широкое изменение. `--no-plugin-dirs` сохраняет обычную конфигурацию `yt-dlp`, но точечно отключает ненужные MP3-сценарию плагины.

## Диагностика

Если предупреждение `[pot:bgutil:http] ... /ping` снова появится при запуске MP3-скрипта:

1. убедиться, что `--no-plugin-dirs` присутствует во всех его вызовах `yt-dlp.exe`;
2. выполнить `yt-dlp.exe --help` и проверить наличие опции `--no-plugin-dirs` в используемой версии;
3. запустить проблемный вызов с `--verbose` и проверить, какие plugin directories и providers фактически загрузились.

> [!warning] Not verified
> После добавления `--no-plugin-dirs` повторный реальный сетевой запуск в рамках этой доработки не выполнялся. Отсутствие `/ping` подтверждено структурой команд и контрактным тестом; окончательный эксплуатационный checkpoint — отсутствие предупреждения при следующем реальном скачивании.
